...
if edit<>"" then
   if db_showSet<>"" then
      db_showSets=split(db_showSet,"|")
      if ubound(db_showSets)>=1 then db_table=db_showSets(1)
      if db_table="" then
      response.Write("<script language=javascript>alert('设置有误!');history.go(-1)</script>") 
      response.end()
      end if
   end if
