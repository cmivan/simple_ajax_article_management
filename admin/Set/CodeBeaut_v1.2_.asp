...
set MMconn=ConfigConn    '用于规则数据绑定
...
set get_filed_conn = server.createobject("adodb.recordset")
	...
	for i=0 to get_filed_conn.fields.count-1
       set AddRs=server.createobject("adodb.recordset")
           AddRsStr="select * from sys_db_fields where db_table='"&db_showSet(1)&"' and db_field='"&get_filed_conn.fields.item(i).name&"'"
...
<td width="30" align="center" class="forumRow">排序</td>
...
%>				
   <tr>
<td align="center" class="forumRow"><input name="field_<%=i%>" type="checkbox" id="field_<%=i%>" value="<%=GNav("db_field")%>" checked="checked" /></td>
<td align="center" class="forumRow"><%=GNav("id")%><input type="hidden" name="fieldID_<%=i%>" value="<%=GNav("id")%>" /></td>
<td class="forumRow" style="padding-left:5px;">
<%=GNav("db_field")%></td>
<td class="forumRow"><input name="field_tip_<%=i%>" type="text" id="field_tip_<%=i%>" style="width:100%" value='<%=GNav("db_title")%>' /></td>
<td align="center" class="forumRow"><input name="field_type_<%=i%>" type="hidden" id="field_type_<%=i%>" value="<%=GNav("db_type")%>" />
...
<tr><td align="center" class="forumRow">
