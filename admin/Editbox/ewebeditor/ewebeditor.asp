...
//function Add_PagIng(){
//alert();
////E_Body=eWebEditor.document.body.innerHTML;
//
////if (E_Body!=""){
////    E_Body=E_Body+"<br>"+"&lt;------PagIng------&gt;"+"<br>";
////    eWebEditor.document.body.innerHTML=E_Body;
////}else{
////alert("请填写内容!再分页.");
////}
//
...
<%if paging="yes" then%>
		<td width=5></td>
		<td class=StatusBarBtnOff id=eWebEditor_VIEW onClick='insert("Add_PagIng");'><img border=0 src="buttonimage/<%=sStyleDir%>/paging.gif" height=15 align=absmiddle></td>
<%end if%>
...
<form id="eWebEditor_UploadForm" action="upload.asp?action=remote&type=remote&style=<%=sStyleName%>" method="post" target="eWebEditor_UploadTarget" style="margin:0">
<input type="hidden" name="eWebEditor_UploadText">
</form>
<iframe name="eWebEditor_UploadTarget" width=500 height=500 style="display:none"></iframe>
