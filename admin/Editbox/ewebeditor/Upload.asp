...
BuildPath Server.MapPath(sUploadDir &year(now())&month(now()))
...
Sub DoUpload_Class()
	...
	oFile.SaveToFile Server.Mappath(sUploadDir & year(now())&month(now())  &"/"& sSaveFileName)
...
Sub InitUpload()
	...
	sAllowExt = Replace(UCase(sAllowExt), "JS", "")
	sAllowExt = Replace(UCase(sAllowExt), "CSS", "")
End Sub
...
Function RelativePath2RootPath(url)
	...
	Do While Left(sTempUrl, 3) = "../"
		...
		sWebEditorPath = Left(sWebEditorPath, InstrRev(sWebEditorPath, "/") - 1)
	Loop
	RelativePath2RootPath = sWebEditorPath & "/" & sTempUrl & year(now())&month(now()) & "/"
End Function
