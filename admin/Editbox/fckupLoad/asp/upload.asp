...
'############# ¿¨¿Í, kake.cc || ¸½¼ÓµÄ ############
  dim sPathFileName_Temps,sPathFileName_Temp,sPathFileName_s
      sPathFileName_Temps = Split(aStyle(1), "|||")
	  sPathFileName_Temp  =sPathFileName_Temps(3)
      sPathFileName_s     =replace(sPathFileName_Temp,"../","")
      
	  sPathFileName_s=sPathFileName_s&sSaveFileName
'##################################################


	Call OutScript("parent.UploadSaved('" & sPathFileName_s & "','" & s_SmallImagePathFile & "');var obj=parent.dialogArguments;if((!obj.eWebEditor)||(!obj.eWebEditor_Temp_HTML)||(!obj.eWebEditor_UploadForm)){obj=parent.dialogArguments.dialogArguments;} try{obj.addUploadFile('" & sOriginalFileName & "', '" & sSaveFileName & "', '" & sPathFileName & "');} catch(e){} " & s_SmallImageScript)
End Sub
