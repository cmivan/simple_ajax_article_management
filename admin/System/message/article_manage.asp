...
<%end if%>
						...
						<td width="115" align="center" bgcolor="#DBF1F7" class="forumRow">修改操作</td>
					</tr>
	  </table>
...
%>
					...
					<tr <%=onTable%>>
					  ...
					  <td width="405" class="forumRow2">&nbsp;<a href="article_edit.asp?id=<%=rs("id")%>">
					    <%if keyword<>"" then%>
...
<%end if%>
					  ...
					  <td width="112" align="center" class="forumRow2"><input name="delete" type="button" value="删除" onClick="javascript:if(confirm('确定要删除此<%=db_title%>吗？删除后不可恢复!')){window.location.href='article_manage.asp?act=del&id=<%=rs("id")%>';}else{history.go(0);}" />
				      <input type="button" name="update" value="查看" onClick="window.location.href='article_edit.asp?id=<%=rs("id")%>' "/></td>
					</tr>
				<%
					rs.movenext
					next
