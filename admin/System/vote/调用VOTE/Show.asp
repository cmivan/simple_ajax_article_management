...
<%set rsTp=server.CreateObject("adodb.recordset")
  sqlTp="select * from Vote_title where id="&request("id")
  rsTp.open sqlTp,connstr%>
  <%set rs=server.CreateObject("adodb.recordset")
    sql="select * from Vote_choice where extends="&request("id")&" order by id desc"
	rs.open sql,connstr%>
	  <%set rsTotle=server.CreateObject("adodb.recordset")
    sqlTotle="select SUM(num) from Vote_choice where extends="&request("id")
        ...
        <tr>
          ...
          <table width="100%"  border="0" cellspacing="4" cellpadding="0">
            <tr>
              ...
              <td width="61%"><img src="Dot.gif" width="<%=int(100*rs("num")/rsTotle(0))%>%" height="15"></td>
